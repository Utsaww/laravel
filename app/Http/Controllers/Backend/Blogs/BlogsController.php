...
class BlogsController extends Controller
{
    ...
    public function create(ManageBlogsRequest $request, Blog $blog)
    {
        ...
        $blogAllCategories = BlogCategory::all()->toArray();
        foreach($blogAllCategories as $blogCategorie) {
            if(!isset($blogCategories[$blogCategorie['type']])) {
                $blogCategories[$blogCategorie['type']] = array("0" => "Select a value");
            }
            $blogCategories[$blogCategorie['type']][$blogCategorie['id']] = $blogCategorie['name'];
        }
